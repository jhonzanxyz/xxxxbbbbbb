...
async def download_notes(app, message, headers, raw_text2, raw_text4, name, name2):
    ...
    for id_text in num_id:
        notes_url = f"https://elearn.crwilladmin.com/api/v1/batch-notes/{raw_text2}?type=notes&topicId={id_text}&chapterId=0"
...
async def careerdl(app, message, headers, raw_text2, raw_text3, prog, name, name1):
    ...
    for id_text in num_id:
        details_url = f"https://elearn.crwilladmin.com/api/v1/batch-detail/{raw_text2}?redirectBy=mybatch&topicId={id_text}&pToken=&chapterId=0"
        try:
            ...
            for item in batch_class:
                ...
                url = f"https://elearn.crwilladmin.com/api/v1/class-detail/{vid_id}"
                try:
                    ...
                    if lessonUrl and lessonExt == 'brightcove':
                        token_url = "https://elearn.crwilladmin.com/api/v1/livestreamToken"
...
async def career_will(app, message):
    ...
    batch_url = "https://elearn.crwilladmin.com/api/v1/my-batch"
    try:
    ...
    topic_url = f"https://elearn.crwilladmin.com/api/v1/batch-topic/{raw_text2}?type=class"
    try:
    ...
    # Fetching notes
    topic_url = f"https://elearn.crwilladmin.com/api/v1/batch-topic/{raw_text2}?type=notes"
    response = requests.get(topic_url, headers=headers)
    response.raise_for_status()
